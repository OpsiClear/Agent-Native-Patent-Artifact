...
| Repository-coupled packages appeared publishable and installed skills mixed host variants | Only `apa-redact` and `@apa/patent-skills` remain public. Public tarballs pack/install/execute in empty projects, host variants bundle independently, and CI blocks on the isolation matrix. | `scripts/check-package-isolation.mjs`; installer/bundle tests; `.github/workflows/gate.yml` |
...
|---|---|---|
| Independently executable distribution | Repository-coupled packages are private; both public tarballs install and execute in empty projects; Claude/Codex/Cursor bundles are independently verified in CI. | Closed |
