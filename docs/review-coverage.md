...
| Standardize PatentSearch / PatentsView / PPS naming | Covered by `docs/source-registry.md` and source IDs: `patentsview`, `uspto-pps`, `google-patents-ui`, `google-bigquery`. `packages/apa-search/sources/index.mjs` now carries checked `referenceUrl`, `lastVerifiedAt`, and `automationPolicy` metadata for each source, and `npm run sources:check` rejects missing or unsafe source-policy entries. |
