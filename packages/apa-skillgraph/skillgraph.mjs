...
export function renderMermaid(graph = loadSkillGraph()) {
  ...
  for (const skill of graph.skills) {
    ...
    if (skill.kind !== "core") {
      for (const hook of asArray(skill.domain_hooks)) {
        lines.push(`  ${nodeId(skill.id)} -. hook:${hook} .-> ${nodeId(hook)}`);
      }
    }
  }
...
export function renderSkillGraphDoc(graph = loadSkillGraph()) {
  ...
  const hookRows = (items) => items.map((s) =>
    `| \`${s.id}\` | \`${s.command}\` | ${s.phase} | ${asArray(s.domain_hooks).map((h) => `\`${h}\``).join(", ") || "-"} | ${s.description} |`
  ).join("\n");
  return [
    "# APA Skill Graph",
    "",
...
export function renderDomainPacksDoc(graph = loadSkillGraph()) {
  return [
    "# APA Domain Packs",
    "",
    "<!-- AUTO-GENERATED by packages/apa-skillgraph/cli.mjs docs; edit domain/skill metadata instead. -->",
    "",
...
export function renderCiBenchmarkingDoc() {
  return [
    "# APA CI Benchmarking",
    "",
    "<!-- AUTO-GENERATED by packages/apa-skillgraph/cli.mjs docs; edit the renderer or benchmark metadata instead. -->",
    "",
