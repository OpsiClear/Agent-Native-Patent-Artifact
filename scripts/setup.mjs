 ...
 * host's skill root (creating `apa-<name>` dirs). No external dependencies; Node >=18.
 *
...
function install(hosts) {
  ...
  for (const h of hosts) {
    mkdirSync(h.abs, { recursive: true });
...
function main() {
  ...
  if (doInstall) {
    ...
    else { console.log("==> installing skills"); install(hosts); }
  } else {
    console.log("  (run with --install to copy skills into the detected host skill root)");
  }
