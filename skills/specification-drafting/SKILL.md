...
## What this does
Generates a candidate specification from the immutable disclosure in `src/embodiments.md`
(`SPEC####` support paragraphs) and the claims, in 37 CFR 1.77 order, with numeral and lexicographic
consistency. The candidate is written under `drafts/specification/` and submitted as an immutable APA
proposal; it does not overwrite the inventor-source disclosure. Every limitation in a claim
...
## Procedure
1. For each claim limitation, identify the source `SPEC####` paragraph and draft proposed additions
   in `drafts/specification/embodiments.md`; never edit `src/embodiments.md` during the agent step.
   Mark candidate paragraphs with `grounding: transcribed` (inventor-sourced) or `reconstructed`
   (drafted from figures), and `defines_numerals` for any numbered element. Preserve `source`,
   `speaker`, and `timestamp` so a reviewer can trace each paragraph to inventor disclosure, figure
   reconstruction, attorney authorship, or human adoption. Under `source_span_policy: strict`, use
   `source_spans: [{path, locator, sha256}]`; paths must be matter-relative under `staging/`,
   `evidence/`, `src/`, `logic/`, or `source/`, and SHA-256 must match the exact source-file bytes.
   Warning/relaxed matters may retain scalar `source_span` + `source_sha256`. Use
   `source: not-recoverable` only when recovery is genuinely impossible.
...
4. **Specification report.** Emit `drafts/specification/specification_report.json` using the shared report schema
   ...
   Then run
   `node packages/apa-reports/cli.mjs check <matter>/drafts/specification/specification_report.json --kind specification`.
5. Submit the candidate through `apa propose` (or MCP `apa_propose`) with
   `artifact_type: specification`. A human reviews and adopts or rejects the exact digest. Only an
   adopted revision may update a compatibility projection.
6. Validate an adopted projection with `node packages/apa-validate/validate.mjs <matter>` (numeral definedness, support edges,
