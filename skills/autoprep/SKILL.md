...
human checkpoint was satisfied. Use the deterministic helper in
`packages/apa-trace/autoprep-state.mjs` for stage completion, hash-based skip checks, restarts, and
`max_examiner_loops` enforcement; do not rely on prompt text alone for resumability.
