...
1. **Claim chart.** For each independent claim, build a table: rows = limitations (`LIM##`), columns =
   the closest `PA##` references (use `logic/reference_matrix.md` and the latest
   `evidence/prior_art/search-dossier-*.json` `analysis_handoff.candidate_cells` if present). Each
   cell must carry `limitation`, `appears_teaches: yes|partial|no|unknown`, a short `quote` or
   "not located", `page_or_para`, `confidence: high|medium|low|unknown`, and `human_verified`.
   Preserve quote/location candidates from the prior-art dossier; do not drop them when translating
   into `/apa-analyze` chart cells. A `yes` or `partial` cell must have an actual quote and location,
   not "not located." Mark teaching as an observation to verify, not a finding.
...
4. **103 (obviousness):** for plausible combinations, name the exact rationale, `rationale_source`
   (`record-evidence`, `common-sense`, `design-need`, `market-pressure`, or `other`), reasonable
   expectation of success, and counter-teaching. Capture inventor-supplied secondary considerations
   (commercial success, long-felt need, unexpected results) only with `nexus` and `evidence_span`.
