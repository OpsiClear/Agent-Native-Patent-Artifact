...
1. **Claim chart.** For each independent claim, build a table: rows = limitations (`LIM##`), columns =
   ...
   `limitation`, `appears_teaches: yes|partial|no|unknown`, a short `quote` or "not located",
   `page_or_para`, `confidence: high|medium|low|unknown`, and `human_verified`. A `yes` or `partial`
   cell must have an actual quote and location, not "not located." Mark teaching as an observation to
