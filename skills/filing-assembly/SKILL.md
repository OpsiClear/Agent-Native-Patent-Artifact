...
- `upload_manifest.json` - SHA-256 hashes for generated source files; separate human-produced upload
  ...
  a location-independent fingerprint of canonical matter inputs that makes changed or legacy packages
  stale;
...
- **User role must be confirmed:** `unknown` is **NO-GO**. Pro-se assembly is neutral collation only and
  must not recommend whether or when to file.
