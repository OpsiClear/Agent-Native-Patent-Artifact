...
3. **Score the judgment dimensions** by reading the artifact and reasoning against the anchors:
   ...
   - **P5 Prior-art Distinction** - is every `distinguished_over` reference substantively distinguished, or does one anticipate?
...
6. **Compute + validate:** `node packages/apa-rigor/cli.mjs check <matter>/patent_rigor_report.json`.
   It schema-validates and computes the verdict: **File-Ready / File-With-Revisions / Major-Rework /
   Do-Not-File** (mean band, but any dimension scoring 1 caps at Do-Not-File, a 2 at Major-Rework).
