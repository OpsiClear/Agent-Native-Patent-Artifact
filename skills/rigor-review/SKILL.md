...
2. **Scaffold:** `node packages/apa-rigor/cli.mjs scaffold --matter <matter> --out <matter>/patent_rigor_report.json`.
   This pre-fills the mechanical dimensions (P3 antecedent basis, P4 support/numeral) from the
   validator and writes `prior_art_state.freshness_summary` identifying the dossier used, search age,
   and closest-art verification state.
...
6. **Compute + validate:** `node packages/apa-rigor/cli.mjs check <matter>/patent_rigor_report.json`.
   ...
   P5 to 2. The check output also prints the prior-art dossier date, search age, and whether closest
   art is human verified.
