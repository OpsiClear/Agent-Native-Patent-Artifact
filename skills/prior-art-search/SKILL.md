...
7. **Update audit and analysis-handoff state.** The search dossier must record the serialized-query
   ...
   candidate `quote_handoff` fields, candidate `rank_explanation` fields,
...
9. **Regression scoring.** When improving this skill, run the fixed retrieval harness:
   `npm run score:prior-art-search`. It scores known-reference recall@20, closest known-reference
   rank, dossier completeness, quote-handoff coverage, and rank-explanation coverage on public
   software-patent fixtures. It is a retrieval-quality metric only, not a legal conclusion or search
   completeness claim.
