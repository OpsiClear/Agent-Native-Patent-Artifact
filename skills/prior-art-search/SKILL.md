...
2. **Access modes.** Only sanctioned API/dataset sources from `docs/source-registry.md` are queried.
   ...
   When a human runs PPS and saves an export/snapshot, import it locally instead of scraping the UI:
   `node packages/apa-search/cli.mjs import-pps-export --matter <matter> --file <pps-export.csv|json|txt> --query "<exact human-entered PPS query>" --reviewer "<name>"`.
   The import command copies and hashes the export under `evidence/prior_art/imports/`, writes a
   search dossier with `human_imports[]`, appends a runlog entry, and leaves all references
   unverified.
...
3. **File the landscape and dossier.** Re-run with `--write` to append `PA##` blocks + write
   ...
   To archive a reference without returning it to the active IDS seed, use one of these canonical
   level-4 headings outside code fences: `#### Rejected PA##`, `#### Removed PA##`,
   `#### Retired PA##`, or `#### Archived PA##` (a title may follow the identifier). An archived
   identifier remains permanently reserved provenance and must never be reused or renumbered, while
   only active `### PA##` entity headings enter the IDS seed. The allocator also reserves identifiers
   found only in canonical/archive evidence records or prior search dossiers and refuses to overwrite
...
7. **Update audit and analysis-handoff state.** The search dossier must record the serialized-query
   ...
   `human_imports[]` entries for any PPS/export handoff files with source file SHA-256 and copied
   artifact path,
